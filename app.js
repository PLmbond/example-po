...
function app() {
  ...
  while (!iExit) {
    ...
    a.records.forEach(r => {
      if (a.suppliers[r.supplier_id])
        r.supplier_id = a.suppliers[r.supplier_id].po_supplier;
      if (a.statusIcons[r.status_id])
        r.status_icon = a.statusIcons[r.status_id];
      if (r.payment_amount)
        r.payment_amount =
          r.status_id == 5 ? (r.payment_amount *= -1) : r.payment_amount;
      r.editable = 1;
      r.iEdit = 0;
      r.iDelete = 0;
      r.iDownload = 0;
